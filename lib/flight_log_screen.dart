...
class FlightLogScreenState extends State<FlightLogScreen>
  ...
  Future<void> _showEditDialog(Map<String, dynamic> itinerary) async {
    ...
    final legs = List<Map<String, dynamic>>.from(
      (itinerary['legs'] as List<dynamic>).map(
    ...
    final fareAmountControllers = legs
        .map(
          (l) => TextEditingController(
            text: (l['fare_amount'] as int? ?? 0) > 0
                ? '${l['fare_amount']}'
                : '',
          ),
        )
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          DropdownButtonFormField<String>(
                            value: fareTypes.contains(fareType)
                                ? fareType
                                : null,
                            ...
                            children: [
                              Expanded(
                                child: DropdownButtonFormField<String>(
                                  value: seatClasses.contains(seatClass)
                                      ? seatClass
                                      : null,
                                  decoration: const InputDecoration(
                                    labelText: '座席クラス',
      ...
      for (final leg in legs) {
        ...
        if (airline == 'JAL') {
          ...
          final appliedRate = (isGoldPremium && anaStatusBonusRate > 0)
              ...
              : (anaCardBonusRate > anaStatusBonusRate
                    ? anaCardBonusRate
                    : anaStatusBonusRate);
          miles = (distance * fareRate * (1 + appliedRate)).toInt();
          points = (distance * fareRate * 2 + (_anaBonusPoint[fareNumber] ?? 0))
              .toInt();
          totalPp += points;
        }
        totalMiles += miles;
