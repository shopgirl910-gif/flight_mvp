...
class _SimulationScreenState extends State<SimulationScreen> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  ...
  final Map<String, String> airportNames = {'HND': '羽田', 'NRT': '成田', 'KIX': '関西', 'ITM': '伊丹', 'NGO': '中部', 'CTS': '新千歳', 'FUK': '福岡', 'OKA': '那覇', 'NGS': '長崎', 'KMJ': '熊本', 'OIT': '大分', 'MYJ': '松山', 'HIJ': '広島', 'TAK': '高松', 'KCZ': '高知', 'TKS': '徳島', 'KOJ': '鹿児島', 'SDJ': '仙台', 'AOJ': '青森', 'AKJ': '旭川', 'AXT': '秋田', 'GAJ': '山形', 'KIJ': '新潟', 'TOY': '富山', 'KMQ': '小松', 'FSZ': '静岡', 'MMB': '女満別', 'OBO': '帯広', 'KUH': '釧路', 'HKD': '函館', 'ISG': '石垣', 'MMY': '宮古', 'UBJ': '山口宇部', 'IWK': '岩国', 'OKJ': '岡山', 'TTJ': '鳥取', 'YGJ': '米子', 'IZO': '出雲', 'NKM': '県営名古屋', 'UKB': '神戸', 'HSG': '佐賀', 'KMI': '宮崎', 'ASJ': '奄美', 'TKN': '徳之島', 'OKI': '隠岐', 'FKS': '福島', 'HNA': '花巻', 'MSJ': '三沢', 'ONJ': '大館能代', 'SHM': '南紀白浜', 'NTQ': '能登', 'KKJ': '北九州', 'TNE': '種子島', 'KUM': '屋久島', 'RNJ': '与論', 'OGN': '与那国', 'HAC': '八丈島', 'MBE': '紋別', 'SHB': '中標津', 'WKJ': '稚内', 'OKD': '丘珠', 'IKI': '壱岐', 'TSJ': '対馬', 'FUJ': '五島福江', 'OIR': '奥尻', 'SYO': '庄内', 'MMJ': '松本', 'AXJ': '天草', 'TJH': '但馬', 'KKX': '喜界', 'KTD': '北大東', 'MMD': '南大東', 'UEO': '久米島', 'TRA': '多良間', 'SHI': '下地島', 'OKE': '沖永良部'};
  ...
  final Map<String, List<String>> fareTypesByAirline = {
    ...
    'ANA': ['運賃1 (150%) プレミアム運賃', '運賃2 (125%) プレミアム小児', '運賃3 (100%) 片道・往復', '運賃4 (100%) ビジネス', '運賃5 (75%) バリュー、株主優待', '運賃6 (75%) トランジット', '運賃7 (75%) スーパーバリュー、いっしょにマイル割', '運賃8 (150%) プレミアム株主', '運賃9 (100%) 普通株主', '運賃10 (70%) 特割プラス', '運賃11 (50%) スマートシニア', '運賃12 (30%) 個人包括'],
  };
  ...
  final Map<String, int> anaBonusPoint = {'運賃1': 400, '運賃2': 400, '運賃3': 400, '運賃4': 0, '運賃5': 400, '運賃6': 200, '運賃7': 0, '運賃8': 0, '運賃9': 0, '運賃10': 0, '運賃11': 0, '運賃12': 0};
  ...
  void initState() {
    ...
    _optDate = '${oneMonthLater.year}/${oneMonthLater.month.toString().padLeft(2, '0')}/${oneMonthLater.day.toString().padLeft(2, '0')}';
  ...
  Future<void> _autoFillFromFlightNumber(int index) async {
    ...
    if (schedules.isEmpty) { setState(() => errorMessage = '$flightNumber便が見つかりません'); return; }
  ...
  Future<Map<String, dynamic>?> _showRouteSelectionDialog(String flightNumber, List<Map<String, dynamic>> schedules) async {
    return showDialog<Map<String, dynamic>>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('$flightNumber便 - 路線を選択', style: const TextStyle(fontSize: 16)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text('同じ便名で複数の路線が見つかりました。', style: TextStyle(fontSize: 13, color: Colors.grey)),
                ...
                child: OutlinedButton(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text('$depTime → $arrTime', style: TextStyle(color: Colors.grey[600], fontSize: 13)),
                    ],
                  ),
                ),
              );
  ...
  Future<void> _saveItinerary() async {
    ...
    if (!isLoggedIn) { 
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('旅程を保存するにはログインが必要です'), backgroundColor: Colors.orange)); 
      return; 
    }
    // Pro版制限チェック
    final proService = ProService();
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('「$title」を$tabNameに保存しました'), backgroundColor: Colors.green));
  ...
  final Map<String, String> airportNameToCode = {
    '羽田': 'HND', '東京(羽田)': 'HND', '成田': 'NRT', '東京(成田)': 'NRT',
    '関西': 'KIX', '大阪(関西)': 'KIX', '伊丹': 'ITM', '大阪(伊丹)': 'ITM',
    '中部': 'NGO', '名古屋(中部)': 'NGO', '新千歳': 'CTS', '札幌(新千歳)': 'CTS',
    '福岡': 'FUK', '那覇': 'OKA', '沖縄(那覇)': 'OKA', '石垣': 'ISG', '宮古': 'MMY',
    '長崎': 'NGS', '熊本': 'KMJ', '大分': 'OIT', '松山': 'MYJ', '広島': 'HIJ',
    '高松': 'TAK', '高知': 'KCZ', '徳島': 'TKS', '鹿児島': 'KOJ', '仙台': 'SDJ',
    '青森': 'AOJ', '旭川': 'AKJ', '秋田': 'AXT', '山形': 'GAJ', '新潟': 'KIJ',
    '富山': 'TOY', '小松': 'KMQ', '静岡': 'FSZ', '女満別': 'MMB', '帯広': 'OBO',
    '釧路': 'KUH', '函館': 'HKD', '北九州': 'KKJ', '宮崎': 'KMI', '奄美': 'ASJ',
    '久米島': 'UEO', '北大東': 'KTD', '南大東': 'MMD', '多良間': 'TRA', '下地島': 'SHI', '沖永良部': 'OKE',
  };

  // JAL運賃名→運賃種別マッピング
  String _mapJALFareType(String fareName) {
    final lower = fareName.toLowerCase();
    if (lower.contains('フレックス') || lower.contains('flex')) return '運賃1 (100%) フレックス等';
    if (lower.contains('株主')) return '運賃2 (75%) 株主割引';
    if (lower.contains('特便') || lower.contains('先得') || lower.contains('セイバー')) return '運賃3 (75%) セイバー';
    if (lower.contains('ウルトラ') || lower.contains('スーパー先得') || lower.contains('スペシャル')) return '運賃4 (75%) スペシャルセイバー';
    if (lower.contains('包括') || lower.contains('ツアー')) return '運賃5 (50%) 包括旅行運賃';
    if (lower.contains('スカイメイト') || lower.contains('当日')) return '運賃6 (50%) スカイメイト等';
    return '運賃3 (75%) セイバー';
  }
  ...
  String _mapANAFareType(String fareName) {
    final lower = fareName.toLowerCase();
    if (lower.contains('プレミアム') && lower.contains('株主')) return '運賃8 (150%) プレミアム株主';
    if (lower.contains('プレミアム')) return '運賃1 (150%) プレミアム運賃';
    if (lower.contains('株主') || lower.contains('優待')) return '運賃9 (100%) 普通株主';
    if (lower.contains('片道') || lower.contains('往復') || lower.contains('フレックス')) return '運賃3 (100%) 片道・往復';
    if (lower.contains('ビジネス')) return '運賃4 (100%) ビジネス';
    if (lower.contains('バリュー') && !lower.contains('スーパー')) return '運賃5 (75%) バリュー、株主優待';
    if (lower.contains('トランジット')) return '運賃6 (75%) トランジット';
    if (lower.contains('スーパーバリュー') || lower.contains('いっしょに')) return '運賃7 (75%) スーパーバリュー、いっしょにマイル割';
  ...
  String _anaSeatClassForFare(String fareType) {
    ...
    if (fareNumber == '運賃1' || fareNumber == '運賃2' || fareNumber == '運賃8') return 'プレミアムクラス';
    return '普通席';
  }
  ...
  List<Map<String, dynamic>> _parseEmailText(String text) {
    ...
    final anaFlightPattern = RegExp(r'\[(\d+)\]\s*(\d{4})年(\d{1,2})月(\d{1,2})日');
    ...
    for (int i = 0; i < anaMatches.length; i++) {
      ...
      // 運賃タイプ
      final fareMatch = RegExp(r'(普通席|プレミアムクラス)\s+[((]?[往復片道]*[))]?([^\s予約]+)').firstMatch(segment);
    ...
    if (!isLoggedIn) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('メールから入力するにはログインが必要です'), backgroundColor: Colors.orange),
        );
  ...
  Future<void> _showEmailImportDialogInternal() async {
    ...
    final result = await showDialog<List<Map<String, dynamic>>>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Row(children: [Icon(Icons.email, color: Colors.purple), SizedBox(width: 8), Text('メールから入力', style: TextStyle(fontSize: 16))]),
        content: SizedBox(
          width: double.maxFinite,
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('JAL/ANAの予約確認メールを貼り付けてください', style: TextStyle(fontSize: 12, color: Colors.grey)),
            const SizedBox(height: 8),
            TextField(controller: controller, maxLines: 10, decoration: const InputDecoration(hintText: 'メール本文をここに貼り付け...', border: OutlineInputBorder(), contentPadding: EdgeInsets.all(8)), style: const TextStyle(fontSize: 11)),
          ]),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('キャンセル')),
    ...
    if (result != null && result.isNotEmpty) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${result.length}件のフライトを読み込みました'), backgroundColor: Colors.green),
        );
      }
  ...
  Future<void> _runOptimization() async {
    ...
    if (!isLoggedIn) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('最適ルート検索にはログインが必要です'), backgroundColor: Colors.orange),
        );
        Navigator.push(context, MaterialPageRoute(builder: (context) => AuthScreen(onAuthSuccess: () {
          Navigator.pop(context);
        })));
      }
      return;
    ...
    setState(() { _optSearching = true; _optError = null; _optResults = []; });
    try {
      final optimizer = PlanOptimizer();
      await optimizer.loadData(_optAirline, _optDate, includeCodeshare: _optIncludeCodeshare);
      ...
      setState(() { _optSearching = false; _optError = 'エラー: $e'; });
    }
  }
      ...
      return SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('条件を設定', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 16),
              // 航空会社 + コードシェア
              Wrap(spacing: 16, runSpacing: 12, children: [
                _optInputSection('航空会社', 120, DropdownButton<String>(
                ...
                _optInputSection('出発空港', 120, DropdownButton<String>(
                  value: _optHomeAirport, isExpanded: true, underline: const SizedBox(),
                  items: majorAirports.map((e) => DropdownMenuItem(value: e, child: Text('$e (${airportNames[e] ?? e})', style: const TextStyle(fontSize: 13)))).toList(),
                  onChanged: (v) => setState(() => _optHomeAirport = v!),
                )),
                _optInputSection('搭乗日', 150, InkWell(
                  onTap: () async {
                    final picked = await showDatePicker(
                      context: context,
                      initialDate: _parseDate(_optDate) ?? DateTime.now().add(const Duration(days: 30)),
                      firstDate: DateTime.now(),
                      lastDate: DateTime.now().add(const Duration(days: 365)),
                      builder: (context, child) => Theme(data: Theme.of(context).copyWith(dialogTheme: const DialogThemeData(insetPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 24))), child: MediaQuery(data: MediaQuery.of(context).copyWith(textScaler: const TextScaler.linear(0.9)), child: child!)),
                    );
                    if (picked != null) {
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      Expanded(child: Text(_optDate.isEmpty ? '日付を選択' : _optDate, style: TextStyle(fontSize: 14, color: _optDate.isEmpty ? Colors.grey : Colors.black87))),
                      Icon(Icons.calendar_today, size: 16, color: Colors.grey[600]),
                    ]),
                  ),
                )),
                _optInputSection('運賃種別', 200, DropdownButton<String>(
                  value: _optFareType.isEmpty ? null : _optFareType, isExpanded: true, underline: const SizedBox(),
                  hint: const Text('選択してください', style: TextStyle(fontSize: 12, color: Colors.grey)),
              ...
              Row(children: [
                ...
                GestureDetector(
                  ...
                  child: Text(
                    _optAirline == 'JAL' ? 'JTA・RAC便も含める' : 'コードシェア便も含める',
          ...
          if (_optResults.isNotEmpty) ...[
            ...
            Text('検索結果', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            ..._optResults.map((plan) => plan.children != null
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          Text('${plan.legCount}レグ', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
        ]),
        const SizedBox(height: 4),
        Row(children: [
          Icon(Icons.access_time, size: 14, color: Colors.grey[600]),
          const SizedBox(width: 4),
          Text('${plan.departureTime} → ${plan.arrivalTime}（${plan.duration}）', style: TextStyle(fontSize: 12, color: Colors.grey[700])),
        ]),
        // フライト詳細
        const SizedBox(height: 8),
        Container(
          ...
          child: Column(children: plan.flights.asMap().entries.map((e) {
            ...
            return Padding(
              ...
              child: Row(children: [
                ...
                Text('${f.departureCode}→${f.arrivalCode}', style: const TextStyle(fontSize: 12)),
                const Spacer(),
    ...
    return Container(
      ...
      child: Theme(
        ...
        child: ExpansionTile(
          ...
          childrenPadding: const EdgeInsets.fromLTRB(8, 0, 8, 8),
          title: Row(children: [
            Text(group.label, style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: color)),
            const Spacer(),
            // 1位の代表値を表示
            if (isFop)
              Text('${_formatNumber(group.totalFop)} FOP', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: color))
            else
              Text(group.duration, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: color)),
          ]),
          children: subChildren.map((ranked) => _buildRankedPlanTile(ranked, color)).toList(),
        ),
      ),
    );
  }
    ...
    return Container(
      ...
      child: Theme(
        ...
        child: ExpansionTile(
          ...
          title: Row(children: [
            Text(plan.label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
            const SizedBox(width: 6),
            Expanded(child: Text(plan.route, style: TextStyle(fontSize: 10, color: Colors.grey[600]), overflow: TextOverflow.ellipsis)),
            const SizedBox(width: 4),
            Text('${_formatNumber(plan.totalFop)} FOP', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: color)),
            const SizedBox(width: 6),
            Text(plan.duration, style: TextStyle(fontSize: 10, color: Colors.grey[600])),
          ]),
          children: [
            // フライト詳細
            Container(
              ...
              child: Column(children: [
                ...plan.flights.asMap().entries.map((e) {
                  ...
                  final f = e.value;
                  return Padding(
                    padding: EdgeInsets.only(bottom: i < plan.flights.length - 1 ? 2 : 0),
                    child: Row(children: [
                      SizedBox(width: 18, child: Text('${i + 1}.', style: TextStyle(fontSize: 10, color: Colors.grey[500]))),
                      SizedBox(width: 50, child: Text(f.flightNumber, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600))),
                      Text('${f.departureCode}→${f.arrivalCode}', style: const TextStyle(fontSize: 11)),
                      const Spacer(),
                      Text('${f.departureTime}-${f.arrivalTime}', style: TextStyle(fontSize: 10, color: Colors.grey[600])),
                      const SizedBox(width: 6),
                      Text('${f.fop}FOP', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: color)),
                    ]),
                  );
                }),
                const SizedBox(height: 4),
                Align(alignment: Alignment.centerRight, child: TextButton.icon(
                  ...
                  label: const Text('修行ログに追加', style: TextStyle(fontSize: 11)),
  ...
  Widget _buildSummaryBar(bool isMobile) {
    if (isMobile) {
      ...
      return Column(children: [Row(mainAxisAlignment: MainAxisAlignment.end, children: [ElevatedButton.icon(onPressed: _showEmailImportDialog, icon: const Icon(Icons.email, size: 14), label: const Text('メールから入力'), style: ElevatedButton.styleFrom(backgroundColor: Colors.orange, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), textStyle: const TextStyle(fontSize: 11))), const SizedBox(width: 8), ElevatedButton.icon(onPressed: _saveItinerary, icon: const Icon(Icons.save, size: 14), label: const Text('保存'), style: ElevatedButton.styleFrom(backgroundColor: Colors.purple, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), textStyle: const TextStyle(fontSize: 11)))]), const SizedBox(height: 8), if (hasJAL) _buildMobileSummaryCard('JAL', Colors.red), if (hasJAL && hasANA) const SizedBox(height: 6), if (hasANA) _buildMobileSummaryCard('ANA', Colors.blue), const SizedBox(height: 10)]);
    }
    return Container(width: double.infinity, padding: const EdgeInsets.all(10), margin: const EdgeInsets.only(bottom: 16),
      ...
      child: Wrap(spacing: 12, runSpacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: [
        ...
        _buildMiniStat('FOP', _formatNumber(jalFOP), Colors.red), _buildMiniStat('マイル', _formatNumber(jalMiles), Colors.red), _buildMiniStat('LSP', _formatNumber(jalLSP), Colors.red),
        _buildMiniStat('レグ', '$jalCount', Colors.red), _buildMiniStat('総額', jalFare > 0 ? '¥${_formatNumber(jalFare)}' : '-', Colors.red), _buildMiniStat('単価', jalUnitPrice != '-' ? '¥$jalUnitPrice' : '-', Colors.red),
        ...
        _buildMiniStat('PP', _formatNumber(anaPP), Colors.blue), _buildMiniStat('マイル', _formatNumber(anaMiles), Colors.blue), _buildMiniStat('レグ', '$anaCount', Colors.blue),
        _buildMiniStat('総額', anaFare > 0 ? '¥${_formatNumber(anaFare)}' : '-', Colors.blue), _buildMiniStat('単価', anaUnitPrice != '-' ? '¥$anaUnitPrice' : '-', Colors.blue),
        Container(width: 1, height: 36, color: Colors.grey[300]),
        ElevatedButton.icon(onPressed: _showEmailImportDialog, icon: const Icon(Icons.email, size: 16), label: const Text('メールから入力'), style: ElevatedButton.styleFrom(backgroundColor: Colors.orange, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), textStyle: const TextStyle(fontSize: 11))),
      ...
      child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(8), border: Border.all(color: color.withOpacity(0.3))),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Spacer(), 
            Text('${count}レグ', style: TextStyle(fontSize: 11, color: color)), 
          ...
          if (isJAL && jalTourPremium) ...[
            ...
            Text('ツアープレミアム適用中', style: TextStyle(fontSize: 10, color: Colors.orange[700], fontWeight: FontWeight.bold)),
          ],
        ])));
  }
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  const SizedBox(height: 8),
                  // JALカード
                  const Text('カード種別', style: TextStyle(fontSize: 11, color: Colors.red)),
                  const SizedBox(height: 4),
                  Container(
                    height: 40,
                    ...
                    child: DropdownButton<String>(
                      ...
                      hint: const Padding(padding: EdgeInsets.only(left: 8), child: Text('選択', style: TextStyle(fontSize: 11))),
                  ...
                  const SizedBox(height: 8),
                  // JALステータス
                  const Text('ステータス', style: TextStyle(fontSize: 11, color: Colors.red)),
                  const SizedBox(height: 4),
                  Container(
                    height: 40,
                    decoration: BoxDecoration(border: Border.all(color: Colors.red.withOpacity(0.3)), borderRadius: BorderRadius.circular(4)),
                  ...
                  Row(children: [
                    ...
                    const SizedBox(width: 8),
                    const Text('ツアープレミアム', style: TextStyle(fontSize: 11, color: Colors.red)),
                  ]),
                ]),
              ),
              ...
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  const SizedBox(height: 8),
                  // ANAカード
                  const Text('カード種別', style: TextStyle(fontSize: 11, color: Colors.blue)),
                  const SizedBox(height: 4),
                  Container(
                    height: 40,
                  ...
                  const SizedBox(height: 8),
                  // ANAステータス
                  const Text('ステータス', style: TextStyle(fontSize: 11, color: Colors.blue)),
                  const SizedBox(height: 4),
                  Container(
                    height: 40,
                    decoration: BoxDecoration(border: Border.all(color: Colors.blue.withOpacity(0.3)), borderRadius: BorderRadius.circular(4)),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: const Row(children: [
                    ...
                    SizedBox(width: 8),
                    Expanded(child: Text('カード未発行の方はこちら', style: TextStyle(fontSize: 11, color: Colors.purple, decoration: TextDecoration.underline))),
                  ]),
                ),
              ),
            ]),
  ...
  Widget _buildMobileExpandedContent(Map<String, dynamic> leg, int legId, int index, int? fop, int? miles, int? lsp, String airline) {
    ...
    return Container(padding: const EdgeInsets.all(8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 6),
      _buildMobileDropdown('運賃種別', leg['fareType'] as String, fareTypesByAirline[airline] ?? [], (v) { if (v != null) { setState(() { legs[index]['fareType'] = v; if (airline == 'ANA') legs[index]['seatClass'] = _anaSeatClassForFare(v); }); _calculateSingleLeg(index); } }),
      const SizedBox(height: 6),
      Row(children: [Expanded(child: _buildMobileDropdown('座席クラス', leg['seatClass'] as String, seatClassesByAirline[airline] ?? [], (v) { if (v != null) { setState(() => legs[index]['seatClass'] = v); _calculateSingleLeg(index); } })), const SizedBox(width: 8), Expanded(child: _buildMobileTextField('運賃(円)', fareAmountControllers[legId]!, '15000', onChanged: (_) => setState(() {})))]),
  ...
  Widget _buildMobileDropdown(String label, String value, List<String> items, void Function(String?) onChanged, {Color? color}) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey[700])), const SizedBox(height: 2),
      Container(height: 36, decoration: BoxDecoration(border: Border.all(color: Colors.grey[300]!), borderRadius: BorderRadius.circular(6)),
        child: DropdownButton<String>(value: currentValue, isExpanded: true, underline: const SizedBox(), icon: Icon(Icons.arrow_drop_down, size: 20, color: Colors.grey[600]), hint: Padding(padding: const EdgeInsets.only(left: 8), child: Text('選択', style: TextStyle(fontSize: 12, color: Colors.grey[500]))), selectedItemBuilder: (c) => items.map((e) => Padding(padding: const EdgeInsets.only(left: 8), child: Align(alignment: Alignment.centerLeft, child: Text(e, style: TextStyle(fontSize: 12, color: getItemColor(e), fontWeight: isAirlineDropdown || color != null ? FontWeight.bold : FontWeight.normal), overflow: TextOverflow.ellipsis)))).toList(), items: items.map((e) => DropdownMenuItem(value: e, child: Text(e, style: TextStyle(fontSize: 12, color: getItemColor(e), fontWeight: isAirlineDropdown ? FontWeight.bold : FontWeight.normal)))).toList(), onChanged: onChanged))]);
  }
  ...
  Widget _buildMobileFlightTimeDropdown(Map<String, dynamic> leg, int legId, int index) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('出発時刻', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey[700])), const SizedBox(height: 2),
      Container(height: 36, decoration: BoxDecoration(border: Border.all(color: Colors.grey[300]!), borderRadius: BorderRadius.circular(6)),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey[700])), const SizedBox(height: 2),
      Container(height: 36, decoration: BoxDecoration(border: Border.all(color: Colors.grey[300]!), borderRadius: BorderRadius.circular(6)),
        child: TextFormField(controller: controller, readOnly: true, style: const TextStyle(fontSize: 12), decoration: InputDecoration(hintText: '選択', isDense: true, border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 10), suffixIcon: Icon(Icons.calendar_today, size: 16, color: Colors.grey[600])),
          onTap: () async { final now = DateTime.now(); final picked = await showDatePicker(context: context, initialDate: _parseDate(controller.text) ?? now, firstDate: DateTime(2020), lastDate: DateTime(2030), locale: const Locale('ja'), builder: (context, child) => Theme(data: Theme.of(context).copyWith(dialogTheme: const DialogThemeData(insetPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 24))), child: MediaQuery(data: MediaQuery.of(context).copyWith(textScaler: const TextScaler.linear(0.9)), child: child!))); if (picked != null) { controller.text = '${picked.year}/${picked.month.toString().padLeft(2, '0')}/${picked.day.toString().padLeft(2, '0')}'; setState(() {}); _fetchAvailableFlights(index); } }))]);
  }
  ...
  Widget _buildDesktopLegCard(BuildContext context, Map<String, dynamic> leg, int index) {
    ...
    return Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: airlineColor.withOpacity(0.3)), boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.1), blurRadius: 4, offset: const Offset(0, 2))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [if (warning != null) Expanded(child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.orange[100], borderRadius: BorderRadius.circular(4)), child: Text(warning, style: TextStyle(fontSize: 11, color: Colors.orange[800])))) else const Spacer(), TextButton.icon(onPressed: _addLeg, icon: const Icon(Icons.add, size: 16), label: const Text('レグ追加'), style: TextButton.styleFrom(foregroundColor: Colors.grey[600], textStyle: const TextStyle(fontSize: 12))), TextButton(onPressed: () => _clearLeg(index, legId), child: Text('クリア', style: TextStyle(color: Colors.grey[600], fontSize: 12))), if (legs.length > 1) IconButton(icon: Icon(Icons.delete_outline, size: 18, color: Colors.grey[600]), onPressed: () => _removeLeg(index), padding: EdgeInsets.zero, constraints: const BoxConstraints(), tooltip: '削除')]),
        const SizedBox(height: 4),
        SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
          _buildDesktopAirlineDropdown(leg, legId, index), const SizedBox(width: 8), _buildDesktopDatePicker('日付', 130, dateControllers[legId]!, context, index), const SizedBox(width: 8), _buildDesktopFlightNumberField(legId, index), const SizedBox(width: 8),
  ...
  Widget _buildDesktopAirlineDropdown(Map<String, dynamic> leg, int legId, int index) {
    ...
    return SizedBox(width: 60, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('航空会社', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)), const SizedBox(height: 4),
  ...
  Widget _buildDesktopFlightTimeDropdown(Map<String, dynamic> leg, int legId, int index) {
    ...
    return SizedBox(width: 70, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('出発時刻', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)), const SizedBox(height: 4),
      Container(height: 32, decoration: BoxDecoration(border: Border.all(color: Colors.grey), borderRadius: BorderRadius.circular(4)),
  ...
  Widget _buildDesktopFareTypeDropdown(Map<String, dynamic> leg, int legId, int index) {
    ...
    return SizedBox(width: 150, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('運賃種別', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)), const SizedBox(height: 4),
      Container(height: 32, decoration: BoxDecoration(border: Border.all(color: Colors.grey), borderRadius: BorderRadius.circular(4)),
        child: DropdownButton<String>(value: currentValue, isExpanded: true, underline: const SizedBox(), menuWidth: 250, hint: const Padding(padding: EdgeInsets.only(left: 6), child: Text('選択', style: TextStyle(fontSize: 10))), selectedItemBuilder: (c) => fareTypes.map((e) => Padding(padding: const EdgeInsets.only(left: 6), child: Align(alignment: Alignment.centerLeft, child: Text(e, style: const TextStyle(fontSize: 10), overflow: TextOverflow.ellipsis)))).toList(), items: fareTypes.map((e) => DropdownMenuItem(value: e, child: Text(e, style: const TextStyle(fontSize: 10)))).toList(), onChanged: (v) { if (v != null) { setState(() { legs[index]['fareType'] = v; if (airline == 'ANA') legs[index]['seatClass'] = _anaSeatClassForFare(v); }); _calculateSingleLeg(index); } }))]));
  }
  ...
  Widget _buildDesktopSeatClassDropdown(Map<String, dynamic> leg, int legId, int index) {
    ...
    return SizedBox(width: 100, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('座席クラス', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)), const SizedBox(height: 4),
      Container(height: 32, decoration: BoxDecoration(border: Border.all(color: Colors.grey), borderRadius: BorderRadius.circular(4)),
        child: DropdownButton<String>(value: currentValue, isExpanded: true, underline: const SizedBox(), menuWidth: 150, hint: const Padding(padding: EdgeInsets.only(left: 6), child: Text('選択', style: TextStyle(fontSize: 10))), selectedItemBuilder: (c) => seatClasses.map((e) => Padding(padding: const EdgeInsets.only(left: 6), child: Align(alignment: Alignment.centerLeft, child: Text(e, style: const TextStyle(fontSize: 10), overflow: TextOverflow.ellipsis)))).toList(), items: seatClasses.map((e) => DropdownMenuItem(value: e, child: Text(e, style: const TextStyle(fontSize: 10)))).toList(), onChanged: (v) { if (v != null) { setState(() => legs[index]['seatClass'] = v); _calculateSingleLeg(index); } }))]));
  }
