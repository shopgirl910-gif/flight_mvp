...
class _SimulationScreenState extends State<SimulationScreen> with AutomaticKeepAliveClientMixin {
  ...
  final validLegs = legs.where((leg) => leg['calculatedFOP'] != null || leg['calculatedPP'] != null).toList();
  if (validLegs.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('計算結果のあるレグがありません'), backgroundColor: Colors.orange),
    );
    return;
  }
  ...
  try {
    ...
    final profile = await Supabase.instance.client
        .from('user_profiles')
        .select('ai_advice_count, ai_advice_date')
        .eq('id', user.id)
        .maybeSingle();

    final today = DateTime.now().toIso8601String().substring(0, 10);
    int count = profile?['ai_advice_count'] ?? 0;
    final lastDate = profile?['ai_advice_date'];
    ...
    if (count >= 3) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('本日のAIアドバイスは上限(3回)に達しました'), backgroundColor: Colors.orange),
      );
      return;
    }

    // レグデータ準備
    final legsData = validLegs.map((leg) {
      ...
      return {
        ...
        'fareAmount': int.tryParse(fareAmountControllers[legId]?.text ?? '') ?? 0,
        'fop': leg['calculatedFOP'] ?? 0,
        'pp': leg['calculatedPP'] ?? 0,
        'miles': leg['calculatedMiles'] ?? 0,
    ...
    final response = await Supabase.instance.client.functions.invoke(
      ...
      body: {
        'legs': legsData,
        'jalCard': selectedJALCard,
        'anaCard': selectedANACard,
        'jalStatus': selectedJALStatus,
        'anaStatus': selectedANAStatus,
        'jalTourPremium': jalTourPremium,
        'targetFop': userProfile?['target_fop'],
        'targetPp': userProfile?['target_pp'],
      },
    );

    if (response.status != 200) {
      throw Exception('AI APIエラー');
    }
    ...
    await Supabase.instance.client.from('user_profiles').upsert({
      'id': user.id,
      'ai_advice_count': count + 1,
      'ai_advice_date': today,
    });

    setState(() {
      aiAdvice = advice;
      aiAdviceRemaining = 2 - count;
...
void _showAiAdviceDialog() {
  showDialog(
    ...
    builder: (context) => AlertDialog(
      title: const Row(children: [
        ...
        SizedBox(width: 8),
        Text('AIアドバイス', style: TextStyle(fontSize: 18)),
      ]),
      content: SizedBox(
        width: double.maxFinite,
        child: SingleChildScrollView(
          child: Text(aiAdvice ?? '', style: const TextStyle(fontSize: 14, height: 1.6)),
        ),
      ),
      actions: [
        Text('残り${aiAdviceRemaining}回/本日', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
        TextButton(
            ...
            Expanded(
              child: isLoadingAdvice
                ...
                : ElevatedButton.icon(
                    ...
                    label: const Text('AI'),
                    style: ElevatedButton.styleFrom(backgroundColor: Colors.deepPurple, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 10)),
                  ),
            ),
          ],
        ),
        ...
        Container(
          ...
          child: Row(children: [
            ...
            isLoadingAdvice
              ...
              : ElevatedButton.icon(onPressed: _getAiAdvice, icon: const Text('🤖', style: TextStyle(fontSize: 14)), label: const Text('AI'), style: ElevatedButton.styleFrom(backgroundColor: Colors.deepPurple, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), textStyle: const TextStyle(fontSize: 11))),
          ]),
        ),
        // 設定パネル（展開時）
  ...
  Widget _buildMobileExpandedContent(Map<String, dynamic> leg, int legId, int index, int? fop, int? miles, int? lsp, String airline) {
    ...
    return Container(padding: const EdgeInsets.all(8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 6),
      _buildMobileDropdown(l10n.fareType, leg['fareType'] as String, fareTypesByAirline[airline] ?? [], (v) { if (v != null) { setState(() => legs[index]['fareType'] = v); _calculateSingleLeg(index); } }),
      const SizedBox(height: 6),
      Row(children: [Expanded(child: _buildMobileDropdown(l10n.seatClass, leg['seatClass'] as String, seatClassesByAirline[airline] ?? [], (v) { if (v != null) { setState(() => legs[index]['seatClass'] = v); _calculateSingleLeg(index); } })), const SizedBox(width: 8), Expanded(child: _buildMobileTextField(l10n.fareAmountYen, fareAmountControllers[legId]!, '15000', onChanged: (_) => setState(() {})))]),
    ...
    return Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: airlineColor.withOpacity(0.3)), boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.1), blurRadius: 4, offset: const Offset(0, 2))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
          ...
          _buildDesktopTextField(l10n.fareAmount, 70, fareAmountControllers[legId]!, '15000', onChanged: (_) => setState(() {})), const SizedBox(width: 8),
