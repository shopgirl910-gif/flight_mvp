...
class PlanOptimizer {
  // 空港別最低乗り継ぎ時間: HND/OKA=30分、その他=20分（JAL公式基準）
  static int _getMinCon(String arrivalAirport) {
    if (arrivalAirport == 'HND' || arrivalAirport == 'OKA') return 30;
    return 20;
  }
  ...
  List<OptimalPlan> findOptimalPlans(String homeAirport, {double fareRate = 0.75, int bonusFop = 400, String airline = 'JAL'}) {
    ...
    if (tourPlans.isNotEmpty) {
      ...
      // レグ数降順、同レグ数ならFOP降順
      tourScored.sort((a, b) => b.totalFop.compareTo(a.totalFop));
  ...
  void _findSimpleRoundTrips(String home, List<List<_Flight>> results) {
    ...
    for (var out1 in outbounds) {
      final returns1 = _getFlightsFrom(
              out1.arrCode, out1.arrMinutes + _getMinCon(out1.arrCode))
      ...
      for (var ret1 in returns1) {
        ...
        final out2s = _getFlightsFrom(home, ret1.arrMinutes + _getMinCon(ret1.arrCode))
            .where((f) => f.arrCode == out1.arrCode)
            .toList();

        for (var out2 in out2s) {
          final returns2 = _getFlightsFrom(
                  out2.arrCode, out2.arrMinutes + _getMinCon(out2.arrCode))
              .where((f) => f.arrCode == home)
              .toList();
  ...
  void _findHubShuttlePlans(String home, List<List<_Flight>> results) {
    ...
    for (var toHub in outbounds) {
      ...
      // ハブからのシャトル先(出発空港以外)
      final shuttleDests = _getFlightsFrom(hub, toHub.arrMinutes + _getMinCon(toHub.arrCode))
          .map((f) => f.arrCode)
  ...
  void _buildShuttlePlan(
    ...
    // HUB→SHUTTLE
    final toShuttles =
        _getFlightsFrom(hub, lastArr + _getMinCon(hub))
    ...
    for (var toS in toShuttles) {
      // SHUTTLE→HUB
      final backToHubs =
          _getFlightsFrom(shuttle, toS.arrMinutes + _getMinCon(toS.arrCode))
      ...
      for (var backH in backToHubs) {
        ...
        // HUB→HOME 帰還便
        final returns =
            _getFlightsFrom(hub, backH.arrMinutes + _getMinCon(backH.arrCode))
                .where((f) => f.arrCode == home)
                .toList();
  ...
  void _findTrianglePlans(String home, List<List<_Flight>> results) {
    ...
    for (var leg1 in outbounds) {
      ...
      final leg2s =
          _getFlightsFrom(mid, leg1.arrMinutes + _getMinCon(leg1.arrCode))
      ...
      for (var leg2 in leg2s) {
        // 直接帰還
        final returns =
            _getFlightsFrom(leg2.arrCode, leg2.arrMinutes + _getMinCon(leg2.arrCode))
                .where((f) => f.arrCode == home)
                .toList();
  ...
  void _tourDFS({
    ...
    final available = _getFlightsFrom(current, currentTime + _getMinCon(current));
